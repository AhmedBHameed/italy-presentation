...
const ItalyMap = (() => {
  ...
  /**
   * The cover: the outline of Italy filled with its own flag, the way the
   * source document opens. The three bands are clipped to the coastline, so
   * the shape reads as a map first and a flag second.
   */
  function drawCoverMap(selector) {
    ...
    const render = () => {
      ...
      const [[x0, y0], [x1, y1]] = p.bounds(outline);
      const bandWidth = (x1 - x0) / 3;

      sel.attr("viewBox", `0 0 ${w} ${h}`);
      sel.selectAll("*").remove();

      // The coastline becomes the clip, and the three bands are painted
      // through it — so the shape reads as a map first and a flag second.
      ...
      const bands = sel.append("g").attr("clip-path", "url(#coverClip)");
      ["flag-green", "flag-white", "flag-red"].forEach((cls, i) => {
        bands
          .append("rect")
          .attr("class", `cover-band ${cls}`)
          .attr("x", x0 + i * bandWidth)
          .attr("y", y0)
          .attr("width", bandWidth + 0.5)
          .attr("height", y1 - y0);
      });

      sel.append("path").attr("class", "cover-outline").attr("d", p(outline));
    };
  ...
  function init(options) {
    ...
    function draw() {
      ...
      layers.regions
        ...
        .on("mousemove", (event, d) => {
          ...
          tooltip.innerHTML =
            (isGuide ? "<small>in the guide</small>" : "") +
            UI.esc(shortName(nameOf(d)));
      ...
      const guideFeatures = geo.features.filter((f) =>
        guideNames.has(nameOf(f)),
      );
      ...
      layers.labels
        ...
        .attr(
          "class",
          (d) => "region-label" + (nameOf(d) === activeName ? " visible" : ""),
        )
      ...
      pins.forEach((pin) => {
        ...
        const taken = new Set(
          placed
            .filter(
              (p) =>
                Math.abs((p.x - x) * k) < 100 && Math.abs((p.y - y) * k) < 34,
            )
            .map((p) => p.slot),
        );
        placed.push({
          ...
          x,
          y,
          slot: SLOT_ORDER.find((s) => !taken.has(s)) || "above",
        });
      });
      return placed;
    }
    ...
    function drawCities(regionName) {
      ...
      const join = layers.cities
        .selectAll("g.city")
        .data(placed, (d) => d.pin.name);
      join.exit().remove();
    ...
    function zoomTo(feature) {
      ...
      const k = Math.max(
        1,
        Math.min(8, 0.78 / Math.max(dx / width, dy / height)),
      );
      svg
        .transition()
        .duration(820)
  ...
  return { load, init, drawCoverMap, shortName };
})();
